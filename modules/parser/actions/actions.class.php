...
/*
	...
	{		
		...
		if(empty($args)) {
			if(!isset($parser["uid"]) || $parser["uid"] != $uid) {
				$result = array('success'=>true,'totalCount'=>1,'rows'=>array(array('message' => "This page has expired! It will be refreshed automatically!", 'redirect' => ($this->hasRequestParameter('referer')?$this->getRequestParameter('referer'):'/'))));
				$result = json_encode($result);
				return $this->renderText($result);
			}	
		} else {
	...
	private static function sortColumn(&$items, $sort, $sort_dir='ASC')
	{
		if($sort)
		{
			RowCmp::sortByColum($items, $sort, $sort_dir);
		}
	}
