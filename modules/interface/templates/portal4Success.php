...
if(!$extjsPortalStateObj)
{
	 ...
	 */
	$config->content[0]['portalColumns'][0][0]->idxml='interface/somepanel';
	
	$config->content[1]['portalLayoutType']='[50,50]';
	/**
	 * unique ids for different widgets on each column
	 * 
	 * $config->content[number_of_item_inside_the_content][column_number][widget_number]->idxml='unique id for xml widget';
	 ...
	 * and 0..n for ExtJsPortalStatePeer::TYPE_TABBED
	 */
	$config->content[1]['portalColumns'][0][0]->idxml='interface/somegrid';
	$config->content[1]['portalColumns'][1][0]->idxml='interface/someform';
...
$layout=new ImmExtjsPortalLayout(array('id'=>'center_panel','tools'=>$toolsA,'idxml'=>$idXml,'layoutType'=>$layoutType));
