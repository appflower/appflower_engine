	...
	public function end()
	{
		if(isset($this->attributes['listeners'])&&count($this->attributes['listeners'])>0)
		{
			foreach ($this->attributes['listeners'] as $type=>$type_params)
			{
				...
				{
					$this->attributes['listeners'][$type]=$this->afExtjs->asMethod(array('parameters'=>$type_params['parameters'],'source'=>implode(null,$type_params['source'])));
				}
				/**
