...
class Util {
    ...
    public static function getMappedValues ($mapped_value = null, $base = null, $group = null, $field = null, $reference = null) {
        ...
        if (class_exists($group_peer) === false || class_exists($base_peer) === false || class_exists($base_query) === false) {
            return false;
        }
        ...
        $starts_with = substr($mapped_value, 0, 2);

        if ($starts_with == 'g-') {
            $mapped_value = str_replace('g-', '', $mapped_value);
            $mapped_value = (int)$mapped_value;

            if ($mapped_value === 0) {
                $query = 'SELECT * FROM '.constant($base_peer.'::TABLE_NAME').' WHERE id NOT IN (SELECT '.$reference.' FROM '.constant($group_peer.'::TABLE_NAME').')';
                $connection = Propel::getConnection(constant($base_peer.'::DATABASE_NAME'));
                $stmt = $connection->prepare($query);
                $stmt->execute();

                $formatter = new PropelObjectFormatter();
                $formatter->setPeer($base_peer);
                $values = $formatter->format($stmt);
            } else {
                $values = call_user_func(array($base_query,'create'))->joinWith($base.'.'.$group, Criteria::INNER_JOIN)
                                                          ->where($group.'.GroupId = ?', $mapped_value)
                                                          ->find();
            }
        } else if ($starts_with == 's-'){
            $mapped_value = (int)preg_replace('/s\-?[0-9]?-/', '', $mapped_value);
            if ($mapped_value > 0) {
                $values = call_user_func(array($base_query,'create'))->findPks($mapped_value);
            }
        } else {
            return false;
        }
        ...
        foreach ($values as $value) {
            $return_values[] = call_user_func(array($value, $method));
        }
    ...
    $groupclass = "NetworkAddressHasGroups",$iclude=true)
    {
    ...
    public static function formatAuditMessage($log_search =null) {
        if (is_array($log_search) === false || null === $log_search) {
            return null;
        }
        ...
        foreach ($log_search as $index => $value) {
		    ...
		    if ($criteria === 'eq') {
		        ...
		        $prefix = '&gt;';
		    } elseif ($criteria === 'ne' || $criteria === 'nc') {
		        $prefix = '!=';
		    } elseif ($criteria === 'starts') {
		        $prefix = '(Starts with)';
		    } elseif ($criteria === 'ends') {
		        $prefix = '(Ends with)';
		    } else {
		        $prefix = '';
		    }
		    ...
		    if (null === $message) {
		        $message = content_tag('div', content_tag('b', sfInflector::camelize($index).': ').$prefix.' '.$value['value'], array('class' => 'ux-grid-filter-info-box'));
		        continue;
		    }
...
//public static function getMappedValues ($mapped_value = null, $base = null, $group = null, $field = null, $reference = null) {
//Util::getMappedValues('s-1', 'NetworkAddressLookup', 'NetworkAddressHasGroups', 'ip', 'group_id');
?>
