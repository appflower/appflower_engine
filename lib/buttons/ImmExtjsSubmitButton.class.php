			...
			if(isset($submitContainerObject->attributes['classic'])&&$submitContainerObject->attributes['classic'])
			{
				...
				$attributes['params']['af_formcfg'] =
					afEditJsonRenderer::buildFormcfg($submitUrl,
						(method_exists($submitContainerObject,"getValidators"))?$submitContainerObject->getValidators():null,
				...
				$source = 'Ext.getCmp("'.$submitContainerObject->attributes['id'].'").getForm().doAction("robustsubmit", '.
					...
					'success'=>$this->immExtjs->asMethod(array(
						...
						*/
						...
						var win;
						/** send a flag that the redirect request is from a popup **/
						
						if(winProp.isPopup && redirect) redirect = Ext.urlAppend(redirect,"fromPopup=true");
