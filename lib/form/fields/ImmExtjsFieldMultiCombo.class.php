 ...
 */
class ImmExtjsFieldMultiCombo extends ImmExtjsField
{
	...
	{		
		...
		if(isset($attributes['plugin']) && $attributes['plugin'] == 'listfield'){
			$this->immExtjs->setAddons(array('js' => array($this->immExtjs->getExamplesDir().'multiselect/ListField.js') ));
		...
		if(isset($attributes['options'])&&count($attributes['options'])>0)
		{
			...
			$this->attributes['data']=$this->immExtjs->asVar(json_encode($options));
	...
	public function processSelectedValues(&$attributes=array())
	{
		...
		if(count($this->selected_values)>0)
		{
			...
			$this->attributes['listeners']['render']['source'].=$this->immExtjs->asVar('field.setValue('.json_encode($this->selected_values).');');
		}
	}
