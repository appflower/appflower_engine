...
class afExtjsGrid 
{
	...
	{		
		...
		*/
		...
		$this->attributes['getWidgetConfig']=$this->afExtjs->asMethod(
			'var o={};
	...
	{
		return new afExtjsGridActions($attributes);		
	}
	...
	{		
		...
		*/	
		...
		if(isset($this->proxy['url'])&&count($this->columns)>0)
		{
					...
					if($this->actionsObject){
						...
						foreach($actions as $key=>$action){							
							if(
								...
								$temp_column['renderer']=$this->afExtjs->asMethod(array(
									"parameters"=>"value, metadata, record",
			...
			$this->attributes[$storePrivateName]['proxy']=$this->afExtjs->HttpProxy(array('url'=>$this->proxy['url'],'method'=>'GET','disableCaching'=>false,
				'listeners'=>array(
					'beforeload'=>$this->afExtjs->asMethod(array(
						'parameters'=>'proxy,params',
						'source'=>'proxy.lastParams=params')),
					'exception'=>self::getJsExceptionListener(
						$this->afExtjs, $this->privateName))
				));
			...
			if(isset($this->proxy['stateId']))
			{
				...
				$this->attributes[$storePrivateName]['listeners']['beforeload']=$this->afExtjs->asMethod(array(
			...
			if($this->attributes['pager'])
			{
				...
				if(isset($this->proxy['stateId']))
				{
					...
					$this->attributes[$pagingToolbarPrivateName]['getState']=$this->afExtjs->asMethod(array(
																				"parameters"=>"",
		...
		$this->attributes['listeners']['mouseover']=$this->afExtjs->asMethod(array(
				"parameters"=>"",
	...
	{
		$this->attributes[$storePrivateName]['sortInfo']=$this->afExtjs->asAnonymousClass(array('field'=>$field,'direction'=>$direction));
	}
	...
	*/
		...
		if(isset($attributes['plugin']) && $attributes['plugin']){
			...
			if(preg_match('/^custom:(.*)$/', $attributes['plugin'], $match)){
				...
				if(file_exists(sfConfig::get('sf_root_dir')."/web/js/custom/".$plugin.".js")){
					$this->afExtjs->setAddons(array('js' => array("/js/custom/".$plugin.".js") ));			
				}else if(file_exists(sfConfig::get('sf_root_dir')."/plugins/appFlowerPlugin/web/js/custom/".$plugin.".js")){
					$this->afExtjs->setAddons(array('js' => array("/appFlowerPlugin/js/custom/".$plugin.".js") ));			
				}
				
				if(file_exists(sfConfig::get('sf_root_dir')."/web/css/".$plugin.".css")){
					$this->afExtjs->setAddons(array('css' => array("/css/".$plugin.".css") ));			
				}else if(file_exists(sfConfig::get('sf_root_dir')."/plugins/appFlowerPlugin/web/css/".$plugin.".css")){
					$this->afExtjs->setAddons(array('css' => array("/appFlowerPlugin/css/".$plugin.".css") ));			
				}			
	...
	private static function getJsExceptionListener($afExtjs, $gridPrivateName) {
		return $afExtjs->asMethod(array(
